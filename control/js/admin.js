...
function fixIt(act, object) {

    if(act=="delete") {
         var reply = confirm("Do you really want to delete "+ object+"?");
        if (reply == true) {
            window.location="../control/delete.php/"+object;
        }
    } else {
        window.location = "../pages/"+act+".php/"+object;
    }      
}
...
function chkForm(formNum) {
    ...
    for (var i = 0; i < intElements; i++){
        ...
        if (document.forms[formNum].elements[i].value == ""){
            document.getElementById("loginWarning").innerHTML = "All fields are required.";
